...
public class JenningsMathewBlackjack {
		...
		while (loop) {
			...
			if (name.equalsIgnoreCase("quit")) {
				...
				while (!done) {
					...
					for (int i = 0; i < 2; i++) {
						int card1 = (int) (Math.random()
								* TOTAL_CARDS_PER_SUITE + 1);
						int suite = (int) (Math.random()
								* TOTAL_NUMBER_OF_SUITES + 1);
						if (card1 == ACE) {
							playersCards += "A";
					...
					if (dealersPoints > BUST && playersPoints > BUST) {
						...
						didWin = "the dealer bust, you won, and you gained "
								+ bet;
						wallet += bet;
					...
					System.out.println(name + ", your cards are\n"
							...
							+ dealersCards + "\nYou had " + playersPoints
							+ " points and the dealer had " + dealersPoints
							+ " points. Therefor " + didWin + ".\nYou have "
							+ moneyFormatter.format(wallet) + " in your bank.");
		...
		while (invalidInput1) {
			...
			while (invalidInput) {
				...
				for (int i = 0; i < bet.length(); i++) {
					if (bet.substring(i, i + 1).equals("0")
							|| bet.substring(i, i + 1).equals("1")
							|| bet.substring(i, i + 1).equals("2")
							|| bet.substring(i, i + 1).equals("3")
							|| bet.substring(i, i + 1).equals("4")
							|| bet.substring(i, i + 1).equals("5")
							|| bet.substring(i, i + 1).equals("6")
							|| bet.substring(i, i + 1).equals("7")
							|| bet.substring(i, i + 1).equals("8")
							|| bet.substring(i, i + 1).equals("9")) {
						invalidInput = false;
					} else {
						invalidInput = true;
						i += bet.length();
	...
	private static Locale getLocale() {
		...
		while (invalidInput) {
			...
			if (l.equals("1") || l.equals("2") || l.equals("3")
					|| l.equals("4") || l.equals("5") || l.equals("6")) {
				invalidInput = false;
			} else {
				invalidInput = true;
				System.out.println("I don't think so!\n");
